...
def main(species_list, taxonomy_filter, data_status, experiment_type, download_option, download_location, processes):
    ...
    if species_list is not None and species_list != '':
        ...
        data_portal = requests.get(
            "https://portal.aquaticsymbiosisgenomics.org/statuses_update/downloader_utility_data_with_species"
            "/?species_list=" + species_list).json()
    else:
        data_portal = requests.get(
            "https://portal.aquaticsymbiosisgenomics.org/statuses_update/downloader_utility_data/?taxonomy_filter=" + taxonomy_filter
            + "&data_status=" + (str(data_status) if data_status is not None else '') + "%20&experiment_type=" + (
                str(experiment_type) if experiment_type is not None else '')).json()
